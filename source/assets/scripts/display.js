function init ()
{
    ...
    if(imageList != null)
    {
        //for each image, create a new image and set the source of that image to the current local storage image path
        //create a new anchor tag and append the new image to it
        //lastly, add the new anchor tag that contains the image to the gallery container that holds all the anchor tags
        //the purpose of this is to allow users to click the image so that they have options presented before them
    ...
    clickedElement.forEach((clickedImg) => 
    {
        clickedImg.addEventListener('click', (event) => 
        {
            ...
            //through a window prompt, ask the user if they would like to edit, delete, or download the clicked image
            let askUser = prompt("What would you like to do with this image: \n(1) Edit \n(2) Delete \n(3) Download");
            //as long as the user inputs an answer to the window prompt
            if(askUser != null)
            {
                //if the user wants to edit the image, this will redirect the user to the edit page
                if(askUser == "1" || askUser.toLowerCase() == "edit")
                {
                    alert("redirecting to edit page");
                }
                //if the user wants to delete the image, the image will be removed from local storage and the gallery
                else if(askUser == "2" || askUser.toLowerCase() == "delete")
                {
                    ...
                    for(let count = 0; count < imageList.length; count++)
                    {
                        ...
                        {
                            //remove it from the local storage array and set it so that it saves the deletion
                ...
                else if(askUser == "3" || askUser.toLowerCase() == "download")
                {
                    ...
                    //create a new anchor tag that will be clicked by the program to automatically download the image
